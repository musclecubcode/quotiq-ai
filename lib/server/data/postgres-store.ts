...
export class PostgresProductionDataStore implements ProductionDataStore {
  ...
  importBrowserData(companyId: string, data: ValidatedBrowserDataImport): Promise<BrowserDataImportResult> {
    return this.tenant(companyId, async (db) => {
      const ids = [...data.clients, ...data.workOrders, ...data.measurements, ...data.notes, ...data.attachments].map(item=>item.id);
      const collision = await db.query(`select id from clients where company_id=$1 and id=any($2::text[]) union all select id from work_orders where company_id=$1 and id=any($2::text[]) union all select id from work_order_measurements where company_id=$1 and id=any($2::text[]) union all select id from work_order_notes where company_id=$1 and id=any($2::text[]) union all select id from work_order_attachments where company_id=$1 and id=any($2::text[]) limit 1`,[companyId,ids]);
      if(collision.rowCount) throw new DataLayerError("CONFLICT","Import would overwrite existing company data.");
      ...
      return {companyId,imported:{clients:data.clients.length,workOrders:data.workOrders.length,measurements:data.measurements.length,notes:data.notes.length,attachments:data.attachments.length},verifiedAt:new Date().toISOString(),localDataRetained:true};
    });
  }
}
