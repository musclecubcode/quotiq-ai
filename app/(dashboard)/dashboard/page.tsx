...
export default function DashboardPage() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <StatCard label="Outstanding Invoices" value={formatCurrency(outstandingInvoices)} icon={IconReceipt} trend="Sent + overdue" iconTone="amber" />
      <StatCard label="Clients" value={String(clients.length)} icon={IconUsers} trend="Secure company workspace" iconTone="green" />
    </div>
    <Card>
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2"><Card><CardHeader title="Estimates" description="Estimate creation is coming during the beta" /><p className="px-5 py-8 text-center text-sm text-slate-500">No estimates yet.</p></Card><Card><CardHeader title="Invoices" description={`${invoices.length} invoice${invoices.length === 1 ? "" : "s"} in your company workspace`} /><div className="px-5 py-8 text-center"><p className="text-sm text-slate-500">{invoices.length === 0 ? "No invoices yet." : `${formatCurrency(outstandingInvoices)} outstanding.`}</p><Link href={invoices.length === 0 ? "/invoices/new" : "/invoices"} className="mt-3 inline-block text-sm font-medium text-blue-700 hover:underline">{invoices.length === 0 ? "Create an invoice" : "View invoices"}</Link></div></Card></div>
  </div>;
}
